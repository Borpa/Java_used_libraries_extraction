...
def calculate_similarity(file_group, file_df):
    ...
    for file_index in file_group:
        ...
        for index, row in file_df.iloc[file_index + 1:].iterrows():
            ...
            if file1_project == file2_project: continue

            file1_name = file1.filename
            file2_name = file2.filename

            file1_deps = file1.dependencies
            file1_deps = [i.replace("'","").strip('][').split(', ') for i in file1_deps]
            file1_deps = list(itertools.chain.from_iterable(file1_deps))

            file2_deps = file2.dependencies
            file2_deps = [i.replace("'","").strip('][').split(', ') for i in file2_deps]
            ...
            result.append([file1_name, file2_name, file1_type, file2_type, ratio, 
                           file1_project, file2_project, file1_path, file2_path])
...
def main():
    ...
    with Pool() as pool:
        result = pool.starmap(calculate_similarity,
                                                  zip(file_groups_nums, repeat(csv_data)))
    
    header = ["filename1", "filename2", "project1_type", "project2_type", "dependency_ratio", 
                           "project1", "project2", "filepath1", "filepath2"]
